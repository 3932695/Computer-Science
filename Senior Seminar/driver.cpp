...
#include <sstream>         // For string and int concat
...
// ofstream outfile( "explosive.txt", ios_base::app ); // These 3 declarations put in main function to personalize for each processor. 
...
int main(int argc, char **argv) {
  ...
  int assignedProcLow;
  int assignedProcHigh;

  cout << " Procs and Numa = " << nprocs << " ___ " << numa[0] << endl;

  if (nprocs <= numa[0]) {
    // cout << "hi" << endl; DELETE
    assignedProcLow = rank * (numa[0] / nprocs);
    assignedProcHigh = (rank + 1) * (numa[0] / nprocs);
  }
  else {
    assignedProcLow = rank;
    assignedProcHigh = rank + 1;
  }    
  
  cout << "Asigned Low" << assignedProcLow << endl;
  cout << "Asigned High" << assignedProcHigh << endl;


  stringstream sstm;
  string tempName = "explosive.txt";
  sstm << rank << tempName;
  tempName = sstm.str();
  const char * tempChar = tempName.c_str();
  ofstream outfile(tempChar, ios_base::app );

  sstm.str("");
  sstm.clear();

  tempName = "numExplosions.txt";
  sstm << rank << tempName;
  tempName = sstm.str();
  tempChar = tempName.c_str();
  ofstream outfile1(tempChar, ios_base::app );

  sstm.str("");
  sstm.clear();

  tempName = "volatility.txt";
  sstm << rank << tempName;
  tempName = sstm.str();
  tempChar = tempName.c_str();
  cout << tempChar << endl;
  ofstream outfile2(tempChar, ios_base::app );
  ...
  int numacount = assignedProcLow;
  
  
  while (numacount > 0) {
    for (int m = 0; m<(assignedProcHigh - assignedProcLow); m++) {
      if(numa[0] > 1) {a[0] += (a[2] - a[1]) / (numa[0]-1);} 
    }
    numacount--;
}
  
  for (int acount = 0; acount < numa[0]; acount++) {	          //5/20/2013 Jim
    if (assignedProcLow <= acount && acount < assignedProcHigh ){
	...
	*/
	...
	incrementGlobals();
      ...
      cout << "forecast rule: " << forecast_rule << " leval: " << leval[0] << "; pupdate: " << pupdate[0] << "; risk: " << a[0] << "; numExplosions: " << numExplosions << endl << forecast_rule << ' ' << leval[0] << ' ' << pupdate[0] << ' ' << a[0] << ' ' << numExplosions << ' '<< upExplosions << ' ' << downExplosions << endl; //wTD number of explosions in 100 runs
      //outfile1 << "leval " << "pupdate " << "numExplosions " << endl; //for counting explosions in 100 runs
      outfile1 << forecast_rule << ' ' << leval[0] << ' ' << pupdate[0] << ' ' << a[0] << ' ' << numExplosions << ' '<< upExplosions << ' ' << downExplosions << endl; //wTD number of explosions in 100 runs Put into output statement above, to avoid multiple processes overlapping
...
void writeToDisk(const dlist &price, const dlist &volatility, const dlist &exbar, 
  ...
  fstream filestr;
  char dataFile[20];
  ofstream outfile500("Simdata.txt", ios_base::app);
  // strcpy(dataFile, "Sim");
  // strcat(dataFile,buffer);
  // strcat(dataFile, "data.txt");
  // filestr.open(dataFile,fstream::out);
  
  for (i = 500; i < rounds[0]-1000; i++) {
    cout << "i = " << i << endl;
    cout << "price = " << price[i] << endl;
    // outfile500 << setprecision(10) << endl;
    // outfile500 << price[i] << endl;
    // outfile500 << volatility[i] << endl;
    // outfile500 << exbar[i] <<endl;
    // outfile500 << axstar[i] <<endl;
    // outfile500 <<  pstar[i] << endl;
    // outfile500 << volume[i] << endl;
    // outfile500 << rates[i] << endl;
    // outfile500 << dividends[i] <<endl;
    
    
    
    // for (i = 0; i < rounds[0]-1000; i++) {
    // 	outfile500 << setprecision(10) << price[i] << " " << volatility[i] << " " << exbar[i] << " " << axstar[i] << " " <<
    // 	  pstar[i] << " " << volume[i] << " " << rates[i] << " " << dividends[i] <<endl;
  }
  //filestr.close();
