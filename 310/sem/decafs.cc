...
S_function * makeFunction(ParseTree *tree, bool prototype)
{
	...
	if(formals){
		for (size_t i = 0; i < formals->children.size();i++) {
			currentFunction->formals.push_back(makeVariable(formals->children[i]));	
		}
	}
...
void pass1(ParseTree * tree)
{
	...
	if (tree->description=="STATEMENT BLOCK") {
...
void pass2(ParseTree * tree){
	...
	if(tree->description == "VARIABLE"){
		...
		if(tree->description == "==== FUNCTION ====")
			stmtblck = tree->children[3];
		S_function * entry = (S_function*)(currentScope->parent->dict[identifier]);
		...
		ParseTree * implements = tree->children[3];
		ParseTree * fields = tree->children[4];
		S_class * entry = (S_class*)(currentScope->parent->dict[identifier]);		
		currentClass = entry;
		...
		if(implements){
			...
			for(size_t i = 0; i < implements->children.size();i++){
				...
				for(int j = 0; j < prototypes.size(); j++){
					...
					vector<S_variable *> proto_parameters = ((S_function *)(prototype))->formals;
						...
						if(currentClass != NULL){
							...
							while(found == false){
								...
								for(int i = 0; i < temp->fields.size(); i++){
									...
									if(temp->fields[i]->name == name && temp->fields[i]->kind() == "S_function"){
										
										result = (S_function*)temp->fields[i];
										found = true;							
									}
								}
					...
					if(!found){
						...
						if(currentScope->local_lookup(name)->kind() != "S_function"){
					...
					for(int k = 0; k < class_parameters.size(); k++){
						...
						S_variable * class_parameter = class_parameters[k];
						S_variable * proto_parameter = proto_parameters[k];
...
bool checkParent(string t1, string t2, Symtab * scope){
	if(scope->lookup(t1) == NULL || scope->lookup(t1)->kind() != "S_class"){
		return false;
	}
...
bool checkParentInterfaces(string t1, string t2, Symtab * scope){
	if(scope->lookup(t1) == NULL || scope->lookup(t1)->kind() != "S_interface"){
		return false;
	}
	/*
	S_interface * subclass = (S_interface*) scope->lookup(t1);
	
	
	while(subclass->parentClass != NULL){
		for(int i = 0; i < subclass->parentClass->interfaces.size(); i++){
			if(subclass->parentClass->interfaces[i]->name == t2){
...
int checkClassInterfaces(S_class * expr_class, string ident, string condition){
		vector<S_interface *> interfaces = expr_class->interfaces;
		
		int pass = -1;
		for(int i = 0; i < interfaces.size(); i++){
			if((interfaces.at(i)->name == ident) && (interfaces.at(i)->kind() == condition)){
		...
		if (pass == -1){
			if(expr_class->parentClass != NULL){
				...
				return checkClassInterfaces(expr_class->parentClass, ident, condition);
			}else{
				return -1;
			}
		}else{
...
void pass3(ParseTree * tree, Symtab * parentScope){
	...
	if(tree->description == "==== CLASS ===="){
		...
		pass3(tree->children[0], parentScope);
		pass3(tree->children[2], parentScope);
		...
		if(op == "="){
			...
			if(t1 != t2){
				...
				if(!(checkParent(t1, t2, parentScope) || checkParent(t2, t1, parentScope))){
					if(!(checkParentInterfaces(t1, t2, parentScope) || checkParentInterfaces(t2, t1, parentScope))){
		...
		pass3(tree->children[1], parentScope);
		...
		pass3(expr, parentScope);
		if(parentScope->lookup(expr->stype) == NULL){
			semantic_error("'" + expr->token->text + "' lacks a type", lineNo);
		}
		
		S_class * expr_class = (S_class *) parentScope->lookup(expr->stype);
		temp = expr_class;
		...
		if(parentScope->lookup(identifier) == NULL || parentScope->lookup(identifier)->kind() != "S_function"){
			semantic_error("function '" + identifier + "' is undefined", lineNo);
		}
		S_function * function = (S_function*) parentScope->lookup(identifier);
		...
		if(parentScope->lookup(identifier) == NULL || parentScope->lookup(identifier)->kind() != "S_class"){
		...
		pass3(typetree, parentScope);
		if(parentScope->lookup(type) == NULL || parentScope->lookup(type)->kind() != "S_class"){
			if(typetree->stype == ""){
		...
		pass3(expr1, parentScope);
		pass3(expr2, parentScope);
		if(expr1->stype == ""){
			semantic_error("type undefined at variable", lineNo);
		}
		...
		pass3(first, parentScope);
		pass3(second, parentScope);
		pass3(third, parentScope);
		...
		if(tokentype == "Identifier"){
			...
			if(parentScope->lookup(tree->token->text)){
				
				semantics * identifier = parentScope->lookup(tree->token->text);
		...
		for (size_t i = 0; i < tree->children.size();i++){
			pass3(tree->children[i], parentScope);
		}
	}
